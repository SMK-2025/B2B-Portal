...
export class NeedsService{
  ...
  create(authHeader:string|undefined,organizationId:string,input:Record<string,unknown>){const user=this.auth.authenticate(authHeader);if(!this.store.memberships.some(m=>m.userId===user.id&&m.organizationId===organizationId))throw new ForbiddenException();const org=this.store.organizations.get(organizationId);if(!org||org.reviewStatus!=="approved")throw new ForbiddenException("Unternehmen nicht freigegeben.");const categoryId=requiredText(input.categoryId,"Kategorie",1,100);if(!this.store.categories.has(categoryId))throw new BadRequestException("Unbekannte Kategorie.");const modes=this.arr(input.deliveryModes) as NeedRecord["deliveryModes"];if(!modes.length||modes.some(x=>!["online","onsite","hybrid"].includes(x)))throw new BadRequestException("Ungültiges Liefermodell.");const need:NeedRecord={id:randomUUID(),organizationId,title:requiredText(input.title,"Titel",5,150),description:requiredText(input.description,"Beschreibung",40,5000),categoryId,requiredSkills:this.arr(input.requiredSkills),preferredIndustries:this.arr(input.preferredIndustries),region:typeof input.region==="string"&&input.region.trim()?input.region.trim():null,deliveryModes:modes,status:"draft",createdAt:new Date().toISOString()};this.store.needs.set(need.id,need);return need;}
